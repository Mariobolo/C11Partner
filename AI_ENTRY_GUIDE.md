# C11Partner AI项目引导文档 v3.0

> ## ⛔ 硬约束（每次接手必须执行，违反会导致补丁式开发循环）
> 
> **在动手改任何一行代码之前，必须完成以下 4 步**：
> 
> | 步骤 | 动作 | 目的 |
> |------|------|------|
> | **1** | `Read` 本文件（AI_ENTRY_GUIDE.md） | 了解项目结构、模块定位 |
> | **2** | `Read` `PROJECT_STATUS.md` 顶部 100 行 | 了解当前进度、已知问题、文档脱节点 |
> | **3** | `Read` `docs/ARCHITECTURE.md` §6（代码组织规范） | 了解 CSS/JS 文件职责和禁止事项 |
> | **4** | `Grep` 关键路径确认现状（如 `!important` 数量、模块文件清单） | **不要相信文档承诺的"已精简至 X 处"，要实际查代码** |
> 
> **完成上述 4 步后才能进入"修改代码"阶段**。如发现文档与代码不符，**先记录在 PROJECT_STATUS.md 顶部的脱节清单**，再继续工作。
> 
> ---
> 
> ### 📋 强制阅读顺序（v3.0 升级）
> 
> 1. **PROJECT_STATUS.md** - 项目当前状态总览（最新进度、已完成、待办）—— **只看顶部 100 行 + 已知问题清单**
> 2. **本文件 (AI_ENTRY_GUIDE.md)** - 项目结构、代码导航、开发规范 —— **整篇**
> 3. **docs/ARCHITECTURE.md** - 整体架构、模块划分、代码组织规范 —— **重点看 §6（规范）和 §7（扩展步骤）**
> 4. **相关技术文档**（根据任务读取 `docs/` 下对应文档）
> 
> ---
> 
> ### 🚨 AI 接手新任务的强制流程
...
>   │
>   ▼
> 步骤1: 读 PROJECT_STATUS.md 顶部
>   │
>   ▼
> 步骤2: 读本文件 (AI_ENTRY_GUIDE.md) 全文
>   │
>   ▼
> 步骤3: 读 docs/ARCHITECTURE.md §6 §7
>   │
>   ▼
> 步骤4: Grep/Glob 验证文档说的对不对
>   │      （!important 处数、模块文件清单、CSS 加载顺序）
>   │
>   ▼
> 步骤5: 如发现文档与代码不符
>   │      → 立刻在 PROJECT_STATUS.md 顶部记录
>   │      → 告知用户"文档承诺 X，实际是 Y"
>   │
>   ▼
> 步骤6: 才开始改代码
> ```
> 
> **违反以上流程导致的问题**（来自 2026-07-07 复盘）：
> - AI 相信文档"已精简至 4 处 !important"，实际是 35 处
> - AI 相信"模块清晰"，实际 index.js 3300+ 行
> - 反复打补丁，每次都从零读代码
